...
namespace TempSensorDevice
{
    ...
    public class SomiodClient
    {
        ...
        public dynamic GetContentInstance(string appName, string containerName, string contentName)
        {
            try
            {
                // Constrói o URL para ir buscar o dado específico
                var response = httpClient.GetAsync(
                    $"{baseUrl}/api/somiod/{appName}/{containerName}/{contentName}"
                ).Result;

                response.EnsureSuccessStatusCode();
                string json = response.Content.ReadAsStringAsync().Result;

                // Devolve o objeto (que inclui o campo 'content')
                return JsonConvert.DeserializeObject<dynamic>(json);
            }
            catch (Exception ex)
            {
                throw new Exception($"Failed to get content-instance: {ex.Message}");
            }
        }
